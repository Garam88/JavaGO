...
1. inbound adapter: HTTP handler, MQ consumer
...
3. adapter 구현(Postgres/Redis/MQ/HTTP) 연결
4. 경계별 에러/타임아웃/로깅 규칙 통일

`go-commerce-api`에 적용하면:

1. 주문 생성 유스케이스는 `OrderRepo`, `InventoryRepo`, `EventPublisher` 포트만 의존
2. HTTP handler는 요청/응답 변환만 담당
3. 인프라 교체(Postgres -> 다른 저장소) 영향이 서비스 코어로 전파되지 않음
