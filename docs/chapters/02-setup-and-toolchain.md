...
macOS에서는 아래 두 가지 방법 중 하나를 사용하면 됩니다.

1. 공식 설치 패키지(`.pkg`) 사용
   - [https://go.dev/dl/](https://go.dev/dl/)에서 macOS 설치 파일을 내려받아 실행
   - 설치 완료 후 새 터미널을 열어 `go version` 확인
2. Homebrew 사용
...
`go` 명령을 찾지 못하면(특히 Apple Silicon 환경) PATH를 확인하세요.
...
아래 명령은 "Go가 제대로 설치되었는지"와 "현재 터미널이 어떤 빌드/모듈 컨텍스트인지"를 확인하기 위한 최소 점검입니다.
...
1. `go version`: Go 설치/버전 확인
2. `GOROOT`, `GOPATH`: Go 도구/워크스페이스 기본 경로 확인
3. `GOMOD`: 현재 디렉터리가 어떤 모듈에 속하는지 확인(비어 있으면 모듈 밖)
4. `GOCACHE`: 빌드/테스트 캐시 위치 확인
5. `GOOS`, `GOARCH`: 현재 타깃 플랫폼 확인
...
참고로 `go.mod`, `go.sum`은 명령어가 아니라 파일입니다.

1. `go.mod`: 모듈 이름과 의존성 버전을 선언하는 파일
2. `go.sum`: 내려받은 모듈의 체크섬(해시)을 기록해 무결성을 검증하는 파일
3. 실제 명령어는 `go mod init`, `go mod tidy`처럼 `go mod ...` 형태입니다.

### 기본 흐름

```bash
go mod init github.com/your-org/go-commerce-api # 현재 디렉터리를 새 Go 모듈로 초기화
go mod tidy                                     # 사용 중인 의존성만 정리해 go.mod/go.sum 갱신
go list -m all                                  # 현재 모듈과 전체 의존성 모듈 목록 출력
```
...
모노레포나 로컬 패키지 테스트 시 `replace`를 쓸 수 있습니다.
...
2. 임시 테스트가 끝나면 제거하고 원래 모듈 버전으로 되돌립니다.
...
특히 표준 라이브러리는 예제 코드의 품질이 좋은 경우가 많아, 먼저 복사해 실행해 보는 것이 좋습니다.
...
- [03. 언어 기초 문법(자바 대비 포인트 중심)](./03-language-basics-for-java.md)
