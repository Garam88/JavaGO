...
1. `string`: 읽기 전용 바이트 시퀀스(관례적으로 UTF-8 텍스트)
2. `[]byte`: 가변 바이트 배열
3. `rune`: 유니코드 코드포인트(`int32` 별칭), 문자 1개를 코드 포인트 값으로 표현한 것

```go
s := "한a"
fmt.Println(len(s))         // 4, 한(3) + a(1), 유니코드 바이트 길이, java 와 차이점 주목
fmt.Println([]rune(s))      // [54620 97]
...
- [05. 컬렉션/문자열/유틸 실전](./05-collections-strings-utils.md)
